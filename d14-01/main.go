...
func main() {
	...
	fmt.Println(calcFuel(f))
}

func calcFuel(r io.Reader) int {
	reaktions := readReactions(r)
	buffer := make(map[string]int)
	v := calcOres(reaktions["FUEL"], reaktions, buffer)
	fmt.Println(buffer)
	return v
}

// calcOres returns the required ores for an reaction
func calcOres(r *reaktion, reaktions map[string]*reaktion, buffer map[string]int) int {
	...
	for _, input := range r.input {
		...
		for needed > 0 {
			count += calcOres(reaktions[input.name], reaktions, buffer)
